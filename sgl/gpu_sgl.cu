...
gpu_data cudaInit(int resolution, int triangles_num, int materials_num, int lights_num, int nodes_num, float* invMatrix, kdNodeLinear *kd_tree, Triangle* scene_triangles_array, Material* materials, PointLight* lights, float* ray_start) {
	...
	cout << "Allocated total of " << total / 1000000 << "MB" << endl;

	cudaMemGetInfo(&free, &total_mem);

	cout << "GPU reports " << free / 1000000 << "MB free of total " << total_mem / 1000000 << "MB" << endl;
...
__host__ void IlluminateKernelCPU(int x, int y, float* ray_start, float *invMatrix, int width, int height, int index, Voxel V, kdNodeLinear *kd_tree, Triangle* scene_triangles_array, Material* materials, PointLight* lights, int lights_len, float* color_buffer, int depth)
{
	...
	*/
	...
	for (int i = 0; i < lights_len; i++)
	{
		...
		RayLinear shadowRay(hit.getX(), hit.getY(), hit.getZ(), lights[i].position.getX(), lights[i].position.getY(), lights[i].position.getZ());


		shadow_data = FindKDIntersection(shadowRay, V, kd_tree, scene_triangles_array, depth);
...
__global__ void IlluminateKernel(float* ray_start, float *invMatrix, int width, int height, Voxel V, kdNodeLinear *kd_tree, Triangle* scene_triangles_array, Material* materials, PointLight* lights, int lights_len, float* color_buffer, int depth)
{
...
void KernelStart(float* ray_start, float *invMatrix, int width, int height, Voxel V, kdNodeLinear *kd_tree, Triangle* scene_triangles_array, Material* materials, PointLight* lights, int lights_len, float* color_buffer, int depth) {
	...
	IlluminateKernel<<< blocks, threads >>>(ray_start, invMatrix, width, height, V, kd_tree, scene_triangles_array, materials, lights, lights_len, color_buffer, depth);
